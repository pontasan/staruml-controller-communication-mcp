# StarUML Controller MCP Server — Communication Diagram

An MCP (Model Context Protocol) server specialized for **Communication Diagrams**. Enables AI assistants like Claude to programmatically create and manage communication diagrams — lifelines and connectors in [StarUML](https://staruml.io/).

<p align="center">
  <img src="images/main.jpg" alt="Communication Diagram — E-Commerce Order Processing" width="800">
</p>

> *Example: E-Commerce Order Processing diagram created entirely via MCP tools — 6 lifelines, 7 connectors with numbered messages, color-coded layer boundaries, notes, and tags.*

## Architecture

<p align="center">
  <img src="images/architecture.svg" alt="Architecture: Claude ↔ MCP (stdio) ↔ staruml-controller-communication-mcp ↔ HTTP REST API ↔ StarUML" width="800">
</p>
...
```bash
git clone https://github.com/pontasan/staruml-controller-communication-mcp.git
cd staruml-controller-communication-mcp
npm install && npm run build
```
...
4. The HTTP server starts — all Communication Diagram tools become available via MCP
...
{
  "mcpServers": {
    "staruml-communication": {
      "command": "node",
      "args": ["/absolute/path/to/staruml-controller-communication-mcp/dist/index.js"]
    }
  }
}
```
...
```bash
claude mcp add staruml-communication node /absolute/path/to/staruml-controller-communication-mcp/dist/index.js
```
...
The core of this server — full CRUD for every Communication Diagram resource.

| Resource | Tools |
|---|---|
| **Diagrams** | `communication_list_diagrams`, `communication_create_diagram`, `communication_get_diagram`, `communication_update_diagram`, `communication_delete_diagram` |
| **Lifelines** | `communication_list_lifelines`, `communication_create_lifeline`, `communication_get_lifeline`, `communication_update_lifeline`, `communication_delete_lifeline` |
| **Connectors** | `communication_list_connectors`, `communication_create_connector`, `communication_get_connector`, `communication_update_connector`, `communication_delete_connector` |
